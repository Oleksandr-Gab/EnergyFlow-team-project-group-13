...
<use href="${$}#heart"></use>
    ...
    <li class="info-item">
        ...
        <p class="modal-pop-descr">${n}/${v}min</p>
    </li>
</ul>


        <p class="description">Description: ${r}</p> 
        </div>`;q.insertAdjacentHTML("afterbegin",g),_()}function _(){S(),w.classList.add("open"),document.body.style.overflow="hidden",T.addEventListener("click",u),document.addEventListener("mouseup",x),document.addEventListener("keydown",I)}function u(){w.classList.remove("open"),T.removeEventListener("click",u),document.removeEventListener("mouseup",x),document.removeEventListener("keydown",I),q.innerHTML="",document.body.style.overflow="",i.removeEventListener("click",l),i.removeEventListener("click",d)}const x=function(e){!document.getElementById("modal").contains(e.target)&&w.classList.contains("open")&&u()},I=function(e){e.key==="Escape"&&u()},j=e=>{const{_id:t,bodyPart:n,burnedCalories:r,description:o,equipment:s,gifUrl:c,name:m,popularity:f,rating:p,target:v,time:g}=e;return{_id:t,bodyPart:n,burnedCalories:r,description:o,equipment:s,gifUrl:c,name:m,popularity:f,rating:p,target:v,time:g}},l=()=>{let e=localStorage.getItem("favoritesCard"),t=[];e!=null&&(t=JSON.parse(e));const n=j(a);t.push(n),localStorage.setItem("favoritesCard",JSON.stringify(t)),S()},d=async()=>{const{_id:e}=a;let t=localStorage.getItem("favoritesCard"),n=JSON.parse(t).filter(r=>r._id!=e);n.length!=0?localStorage.setItem("favoritesCard",JSON.stringify(n)):localStorage.removeItem("favoritesCard"),S()},S=()=>{const{_id:e}=a;let t=localStorage.getItem("favoritesCard");t||(i.addEventListener("click",l),i.removeEventListener("click",d),i.innerHTML=`Add to favorites ${L}`),t!=null&&JSON.parse(t).forEach(n=>{n._id==e?(i.innerHTML=`Remove favorite ${L}`,i.removeEventListener("click",l),i.addEventListener("click",d)):(i.removeEventListener("click",d),i.addEventListener("click",l),i.innerHTML=`Add to favorites ${L}`)})},K=100,h=document.querySelector(".skroll-btn"),z=()=>window.scrollY||document.documentElement.scrollTop;window.addEventListener("scroll",()=>{z()>K?h.classList.add("skroll-btn-active"):h.classList.remove("skroll-btn-active")});h.addEventListener("click",()=>{window.scrollTo({top:0,behavior:"smooth"})});export{G as a,$ as i};
//# sourceMappingURL=skroll-btn-589602e5.js.map
