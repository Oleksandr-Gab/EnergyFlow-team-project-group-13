...
function M(e){const t=[e];$.innerHTML=t.reduce((r,{quote:o,author:s})=>r+`<p class="quote-text">${o}</p>
    ...
    </li>`,""))}const j=document.querySelector(".footer-form"),C=document.querySelector(".footer-form-input"),D="https://energyflow.b.goit.study/api/subscription";j.addEventListener("submit",G);async function G(e){e.preventDefault();const t=e.currentTarget.elements.footerInput.value.trim();try{const r=await d.post(D,{email:t});if(r.status>=200&&r.status<300){const o=r.data.message;p.success({title:"OK",message:o,color:"white",position:"center"})}else throw new Error}catch(r){p.error({title:"Error",message:r.response.data.message,position:"center",backgroundColor:"gray"})}finally{C.value=""}}const c={modalBackdrop:document.querySelector(".js-backdrop-modal"),closeBtn:document.querySelector(".js-rating-close"),form:document.querySelector(".js-rating-form"),rateStars:document.querySelector(".js-stars-list"),rateValue:document.querySelector(".js-rating-data"),star:document.querySelectorAll(".js-rating-star")};let w=0;window.addEventListener("click",u);document.addEventListener("keydown",H);c.closeBtn.addEventListener("click",U);c.form.addEventListener("submit",k);c.rateStars.addEventListener("click",W);function u(e){e.target.classList.contains("backdrop")&&(c.modalBackdrop.classList.remove("is-hidden"),window.removeEventListener("click",u))}function U(e){e.currentTarget&&(c.modalBackdrop.classList.add("is-hidden"),window.removeEventListener("click",u))}function H(e){e.code==="Escape"&&(c.modalBackdrop.classList.add("is-hidden"),window.removeEventListener("click",u))}function W(e){let t=e.target,r=c.rateStars.querySelector(".star-active");t.classList.add("star-active"),r&&r.classList.remove("star-active"),setTimeout(()=>{c.rateValue.textContent=`${t.dataset.rate}.0`},0),w=t.dataset.rate}function k(e){e.preventDefault(),console.log({rating:w,email:e.target.elements.email.value,comment:e.target.elements.comment.value}),c.modalBackdrop.classList.remove("is-hidden"),c.form.removeEventListener("submit",k)}
//# sourceMappingURL=main-8e024e5b.js.map
