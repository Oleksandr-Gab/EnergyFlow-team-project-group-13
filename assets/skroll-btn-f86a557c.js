...
<use href="${$}#heart"></use>
    ...
    <li class="info-item">
        ...
        </div>`;q.insertAdjacentHTML("afterbegin",g),U()}function U(){S(),w.classList.add("open"),document.body.style.overflow="hidden",T.addEventListener("click",u),document.addEventListener("mouseup",x),document.addEventListener("keydown",I)}function u(){w.classList.remove("open"),T.removeEventListener("click",u),document.removeEventListener("mouseup",x),document.removeEventListener("keydown",I),q.innerHTML="",document.body.style.overflow="",i.removeEventListener("click",l),i.removeEventListener("click",d)}const x=function(e){!document.getElementById("modal").contains(e.target)&&w.classList.contains("open")&&u()},I=function(e){e.key==="Escape"&&u()},_=e=>{const{_id:t,bodyPart:s,burnedCalories:r,description:o,equipment:n,gifUrl:c,name:m,popularity:p,rating:f,target:v,time:g}=e;return{_id:t,bodyPart:s,burnedCalories:r,description:o,equipment:n,gifUrl:c,name:m,popularity:p,rating:f,target:v,time:g}},l=()=>{let e=localStorage.getItem("favoritesCard"),t=[];e!=null&&(t=JSON.parse(e));const s=_(a);t.push(s),localStorage.setItem("favoritesCard",JSON.stringify(t)),S()},d=async()=>{const{_id:e}=a;let t=localStorage.getItem("favoritesCard"),s=JSON.parse(t).filter(r=>r._id!=e);s.length!=0?localStorage.setItem("favoritesCard",JSON.stringify(s)):localStorage.removeItem("favoritesCard"),S()},S=()=>{const{_id:e}=a;let t=localStorage.getItem("favoritesCard");t||(i.addEventListener("click",l),i.removeEventListener("click",d),i.innerHTML=`Add to favorites ${L}`),t!=null&&JSON.parse(t).forEach(s=>{s._id==e?(i.innerHTML=`Remove favorite ${L}`,i.removeEventListener("click",l),i.addEventListener("click",d)):(i.removeEventListener("click",d),i.addEventListener("click",l),i.innerHTML=`Add to favorites ${L}`)})},j=100,h=document.querySelector(".skroll-btn"),K=()=>window.scrollY||document.documentElement.scrollTop;window.addEventListener("scroll",()=>{K()>j?h.classList.add("skroll-btn-active"):h.classList.remove("skroll-btn-active")});h.addEventListener("click",()=>{window.scrollTo({top:0,behavior:"smooth"})});export{Y as a,$ as i};
//# sourceMappingURL=skroll-btn-f86a557c.js.map
