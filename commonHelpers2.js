import{a as Y}from"./assets/modal-form-80233cd8.js";import{a as h,i as s}from"./assets/vendor-29bef8ec.js";const K=document.querySelector(".filter-list"),S=document.querySelector(".gallery"),y=document.querySelector(".pagination-btn"),Z=document.querySelector(".waist"),Q=h.create({baseURL:"https://energyflow.b.goit.study/api"});let x;const H=document.querySelector('button[name="Muscles"]');let B;document.addEventListener("DOMContentLoaded",async()=>{await T({filter:"Muscles"}),H.classList.add("active"),B=document.querySelector(".pg-num-btn"),B.classList.add("pg-num-btn-active")});const J=async({filter:e,page:t,limit:r})=>{try{return x=await Q.get("/filters",{params:{filter:e,page:t,limit:r}}),x}catch(n){console.error(n),s.error({message:n.message,color:"red",position:"topCenter"})}};K.addEventListener("click",e=>{e.preventDefault(),S.innerHTML="",y.innerHTML="",Z.innerHTML="",e.target.tagName==="BUTTON"&&(H.classList.remove("active"),T({filter:e.target.name}))});y.addEventListener("click",e=>{e.preventDefault(),e.target.tagName==="BUTTON"&&(y.innerHTML="",S.innerHTML="",T({filter:e.target.name,page:e.target.id}))});async function T({filter:e,page:t=1,limit:r=12}){try{const n=await J({filter:e,page:t,limit:r}),a=()=>{const g=n.data.totalPages;let o="";for(let i=1;i<=g;i++)o+=`<button id="${i}" class="pg-num-btn" type="button" name="${e}"
 >${i}</button>`;return o},d=n.data.results.reduce((g,{name:o,filter:i,imgUrl:_})=>g+`<li class="gallery-item" id=${o}>
          ...
          </li>`,""),v=a();y.insertAdjacentHTML("afterbegin",v),S.insertAdjacentHTML("afterbegin",d)}catch(n){console.error(n),s.error({message:n.message,color:"red",position:"topCenter"})}}const X=document.querySelector(".search-block"),ee=document.querySelector(".custom-pagination-btn"),b=document.querySelectorAll(".render-pagination-btn"),te=h.create({baseURL:"https://energyflow.b.goit.study/api/"}),re={fetchFilters:async e=>{try{return(await te.get("filters",{params:e})).data.results}catch(t){console.error(t),ne(t)}}},ne=e=>{throw s.error({title:"API Error",message:"An error occurred while fetching data from the API."}),e},se=async e=>{try{return(await re.fetchFilters({...e})).map(ae).join("")}catch(t){return console.error(t),O(),""}},ae=e=>`<li class="render-page-one-item" data-name="${e.name}">
          ...
          </li>`,O=()=>{s.error({title:"Rendering Error",message:"An error occurred while rendering data."})},oe=async e=>{try{const t=await e;X.innerHTML=t}catch(t){console.error(t),O()}},ie=e=>{const t=parseInt(e.dataset.btn)===3?1:-1;b.forEach(r=>{r.textContent=parseInt(r.textContent)+t}),b[0].style.display=f.page>1?"block":"none",b[2].style.display=f.page>=de-1?"none":"block"},ce=e=>{e.target.nodeName==="BUTTON"&&(f.page=parseInt(e.target.textContent),oe(se(f)),ie(e.target))},le=getComputedStyle(document.querySelector("body")).width,R=le<768?8:12,f={limit:R,page:1},de=69/R;ee.addEventListener("click",ce);const ue="Unfortunately, no results were found. You may want to consider other search options to find the exercise you are looking for. Our range is wide and you have the opportunity to find more options that suit your needs.",M=document.querySelector(".gallery"),q=document.querySelector(".section-title");document.querySelector("#slash");const L=document.querySelector(".waist"),E=document.querySelector("#search"),U=innerWidth,k=h.create({baseURL:"https://energyflow.b.goit.study/api"}),F=async(e,{params:t})=>await k.get(e,{params:t});M.addEventListener("click",e=>{e.preventDefault(),L.innerHTML="",M.innerHTML="",E.style.display="block",L.classList.add("information-cards");const r=e.target.id.split(":");k.defaults.params={limit:U>1400?"9":"8",muscles:r[0]==="Muscles"?r[1]:null,bodypart:r[0]==="Body parts"?r[1]:null,equipment:r[0]==="Equipment"?r[1]:null},q.insertAdjacentHTML("beforeend",`<p id="slash">&#8260;<span class="title-span">${r[1]}</span></p>`),console.log(q),F("/exercises",{params:{}}).then(n=>{D(n.data.results)}).catch(n=>{s.error({message:n.message,position:"topRight"})})});let p,A;E.addEventListener("input",e=>{clearTimeout(A),A=setTimeout(function(){p=E.value,console.log(p)},1e3),searchBlock.addEventListener("click",t=>{if(console.log(p.toLowerCase()),p.trim()!=="input-value"){M.innerHTML=`<div class="errorEmageContainer">${ue}</div>`;return}k.defaults.params={limit:U>1400?"9":"8"},F("/exercises",{params:{keyword:t.target.value}}).then(r=>{console.log(r.data),D(r.data.results)}).catch(r=>{s.error({message:r.message,position:"topRight"})})})});function D(e){L.insertAdjacentHTML("afterbegin",e.reduce((t,{burnedCalories:r,name:n,bodyPart:a,rating:d,time:v,target:g,_id:o})=>t+`<li class="gallery-card">
